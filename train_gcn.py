# --- START OF FILE train_gcn_regression.py ---
import torch
...
from gcn_regression_model import GCNRegression
from dataset_gcn import GCNGraphDataset # Use the GCN specific dataset

# --- Evaluation Metrics (same as in train_regression.py) ---
...
def calculate_pearsonr(y_true, y_pred):
    ...
    if len(y_true_np) < 2 or len(y_pred_np) < 2: return 0.0 # pearsonr needs at least 2 points
    try:
...
# --- Training and Evaluation Functions (same as in train_regression.py, adapt if model output changes) ---
def train_epoch(model, loader, criterion, optimizer, device):
    ...
    for batch in loader:
        ...
        optimizer.zero_grad()
        predictions = model(batch) # Model expects PyG Batch object
...
def evaluate(model, loader, criterion, device):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            loss = criterion(predictions, target) # Ensure target is squeezed
            total_loss += loss.item() * batch.num_graphs
            processed_graphs += batch.num_graphs
...
def main(args):
    ...
    print("Initializing GCN datasets...")
    ...
    print(f"Model input features: {in_channels}, Edge features: {edge_dim if edge_dim else 'Not Used'}")
    ...
    model = GCNRegression(
        ...
        edge_dim=edge_dim, # Pass actual edge dimension if using edge features
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_metrics["loss"] < best_val_loss:
            ...
            print(f"*** Best GCN model saved at epoch {epoch} ***")
    ...
    if os.path.exists(best_model_path):
        ...
        print(f"Best GCN model from epoch {checkpoint['epoch']} loaded for test evaluation.")
        test_metrics = evaluate(model, test_loader, criterion, device)
        print("\n--- GCN Test Set Performance (Best Model) ---")
        for metric, value in test_metrics.items(): print(f"{metric.upper()}: {value:.4f}")
    else:
        print("Warning: Best GCN model checkpoint not found.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='GCN for Binding Affinity Regression')
    # Data args
    ...
    parser.add_argument('--checkpoint_dir', type=str, default='./checkpoints_gcn', help='Directory for checkpoints')
    parser.add_argument('--model_name', type=str, default='gcn_regression', help='Name for model checkpoints')
    parser.add_argument('--graph_type', type=str, default='Graph_GCN', help='Prefix for saved graph files')
    ...
    parser.add_argument('--target_col', type=str, default='-logkd/ki', help='Target value column in CSV') # Match your CSV
    ...
    # Model args
    parser.add_argument('--hidden_channels', type=int, default=128, help='GCN hidden channels')
    parser.add_argument('--num_layers', type=int, default=3, help='Number of GCN layers')
    parser.add_argument('--dropout', type=float, default=0.2, help='Dropout rate')
    parser.add_argument('--use_edge_features', action='store_true', default=True, help='Use edge features (implies GINE-like model)')
    parser.add_argument('--global_pool', type=str, default='mean', choices=['mean', 'add'], help='Global pooling')

    args = parser.parse_args()
    main(args)
# --- END OF FILE train_gcn_regression.py ---